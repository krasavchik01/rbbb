...
export function dedupeProjectFiles(files: any[] = []): any[] {
  ...
  for (const file of files || []) {
    ...
    byKey.set(key, {
      ...
      fileType: file.fileType || file.type || byKey.get(key)?.fileType || 'application/octet-stream',
      type: file.type || file.fileType || byKey.get(key)?.type || 'application/octet-stream',
      fileSize: Number(file.fileSize ?? file.size ?? byKey.get(key)?.fileSize ?? 0),
      size: Number(file.size ?? file.fileSize ?? byKey.get(key)?.size ?? 0),
      uploadedAt: file.uploadedAt || file.uploaded_at || byKey.get(key)?.uploadedAt || '',
      uploadedBy: file.uploadedBy || file.uploaded_by || byKey.get(key)?.uploadedBy || 'system',
      storagePath: file.storagePath || file.path || byKey.get(key)?.storagePath,
      isSeafile: file.isSeafile || file.source === 'seafile' || file.storage === 'seafile' || byKey.get(key)?.isSeafile || false,
