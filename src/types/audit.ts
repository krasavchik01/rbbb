 ...
 * с адаптацией под Национальные стандарты аудита РК (НСА РК)
 *
 * Структура согласно:
 * - IAASB Handbook 2023-2024 (МСА)
 * - Закон РК "Об аудиторской деятельности" от 20.11.1998 № 304-I
 * - Национальные стандарты аудита РК (НСА РК 1-999)
 *
 * Иерархия:
 * 1. Стандарт МСА/НСА РК (ISA 200-810 / НСА РК 200-810)
...
// ============================================
// НАЦИОНАЛЬНЫЕ СТАНДАРТЫ АУДИТА РК
// ============================================

export type KZAuditStandardCode =
  // Специфичные требования РК
  | 'KZ_TAX'      // Требования налогового законодательства РК
  | 'KZ_FINREP'   // Требования к финансовой отчетности по РК
  | 'KZ_BANK'     // Требования НБ РК для банков
  | 'KZ_INSURANCE' // Требования для страховых организаций
  | 'KZ_PENSION'   // Требования для ЕНПФ
  | 'KZ_SUBSOIL'   // Требования недропользования
  | 'KZ_LABOR'     // Трудовое законодательство РК
  | 'KZ_SOCIAL'    // Социальное страхование РК
  | 'KZ_ANTIMON';  // Антимонопольное законодательство РК

export interface KZAuditStandard {
  code: KZAuditStandardCode;
  nameRu: string;
  nameKz: string; // Казахский язык
  description: string;
  legalBasis: string; // Ссылка на закон РК
