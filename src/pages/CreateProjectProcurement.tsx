...
export default function CreateProjectProcurement() {
  ...
  const handleSubmit = async () => {
    ...
    const configuredStages = hasStages && projectStages.length > 0
      ? projectStages.map((stage) => ({
          ...stage,
          auditPeriodId: undefined,
        }))
      : [];
    ...
    const project = {
      ...
      // Один договор / один срок оказания услуг = один проект.
      // Этапы принадлежат самому проекту и не создают вложенные сущности.
        ...
        {hasStages && (
          <>
            <p className="mb-3 text-sm text-muted-foreground">
              Этапы — необязательные шаги работ внутри этого проекта. Новый договор, год или отдельный аудит оформляйте новым проектом.
            </p>
            <ProjectStagesEditor
